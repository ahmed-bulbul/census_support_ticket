...
export class MenuListComponent implements OnInit {
  ...
  _getAuthResData(){
    const apiURL = this.baseUrl + '/acl/AuthResourceDto/resource';
    const queryParams: any = {};
    // const params = this.getUserQueryParams(this.configPgn.pageNum, this.configPgn.pageSize);
    queryParams.entityName='SystemMenu';
    ...
    this.systemService.sendGetRequest(apiURL, queryParams).subscribe(
      (response: any) => {
        if(response.status === true){
          if(response.data){
            this._setActionAuth(response.data);
          }
  ...
  _setActionAuth(resAuthData){
    // set action auth
    this.authObj.create = resAuthData.create;
    this.authObj.read = resAuthData.read;
    this.authObj.update =resAuthData.update;
    this.authObj.delete =resAuthData.delete;
  }
  deleteEntityData(id: any){
    const apiURL = this.baseUrl + '/system/systemMenu/delete/' + id;
    console.log(apiURL);
    ...
    this.spinnerService.show();
    this.systemService.sendDeleteRequest(apiURL, formData).subscribe(
      (response: any) => {
  ...
  _getListData(){
    const apiURL = this.baseUrl + '/system/systemMenu/getLists';
