...
declare var $: any;
@Component({
  selector: 'app-tire2-list',
  templateUrl: './tire2-list.component.html',
  styleUrls: ['./tire2-list.component.css']
})
export class Tire2ListComponent implements OnInit {
  ...
  _initSolveForm() {
    this.myForm = this.formBuilder.group({
      tier2SolutionType: ['', [Validators.required]],
      tier2SolutionDescription: ['', [Validators.required]],
    });
  }
  ...
  getListData() {
    ...
    const apiURL = this.baseUrl + '/ticket/tire2/getList';

    let queryParams: any = {};
    this.problemCategory = "TECHNICAL";
    this.status = "SEND_TO_T2";
    this.tier2ReceiveBy = this.loginService.getUser().username;
  ...
  ticketReceive(id)
  {
    const apiURL = this.baseUrl + '/ticket/tire2/stsUpdate/' + id;
    console.log(apiURL);
  ...
  ticketSolve(solveId)
  {
    ...
    const apiURL = this.baseUrl + '/ticket/tire2/solveTicket/' + solveId;
  ...
  {
    const apiURL = this.baseUrl + '/ticket/tire2/terminateTicket/' + terminateId;
    const formData: any = {};
